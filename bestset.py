...
def plot(highlight=[]):
    ...
    plt.plot(Nc, robust, "ko", markersize=3, alpha=0.7, label="Gmode Tests ($G_{q_{1}}, upperlimit$)")
    ...
    if len(highlight) != 0:
       for n, texto in enumerate(name):
           if all([True if item[0] == q1[n] and item[1] == ulim[n] else False for item in highlight]):
              ...
              plt.text(Nc[n], robust[n], texto, fontsize=10, fontweight='black', style='italic')
